...
def train(args, dataloaders):
    ...
    for step in step_iter:
        ...
        if ((step + 1) > args.eval_after and (step + 1) % args.eval_every == 0 and (step + 1) % steps_in_epoch == 0) or (step + 1) == num_steps:
            ...
            val_results, labels_vs_preds, step_results, y_true_all, y_pred_all = eval_model(args, net, private_clients,
            ...
            logger.debug(f'Back from evaluation. acc {val_avg_acc}')
            ...
            logger.debug(f'Epoch ended at step {step + 1}')
            logger.debug(f'Epoch: {(step // steps_in_epoch) + 1}')
