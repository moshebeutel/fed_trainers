...
from gep_utils import embed_grad, project_back_embedding, add_new_gradients_to_history, \
    compute_subspace, train_nonlinear_dim_reduction, nonlinear_embed_grad, nonlinear_project_back_embedding
...
def train(args, dataloaders):
    ...
    for step in step_iter:
        ...
        train_nonlinear_dim_reduction(args, [train_loaders[c_id] for c_id in public_clients], net)
        ...
        for j, c_id in enumerate(client_ids_step):
            ...
            # get client grads
            for n, p in local_net.named_parameters():
                grads[n].append(p.data.detach() - prev_params[n].to(device))
        ...
        embedded_grads = nonlinear_embed_grad(grads_flattened).to(device)
        ...
        reconstructed_grads = nonlinear_project_back_embedding(noised_embedded_grads, device).squeeze()
        ...
        net = load_aggregated_grads_to_global_net(aggregated_grads.to('cpu'), net, prev_params, global_lr)
