                                ...
                                for num_public_clients in [5]:
                                    ...
                                    for history_size in [50, 100]:
                                        ...
                                        # for grad_clip in [1.0]:
                                            print(f'@@@ Run gep_public_no_gp lr {lr} sigma {sigma} history_size {history_size} grad_clip {grad_clip} %%%')


                                            sample_prob = float(num_clients) / float(num_client_agg)
                                            num_steps = math.ceil(num_epochs * sample_prob)
                                            subprocess.run(['poetry', 'run', 'python', f'trainer_{script_name}_gep_public_no_gp.py',
                                                            '--data-name', data_name,
